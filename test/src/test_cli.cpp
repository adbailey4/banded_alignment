...
TEST_F(CliTest, help)
{
    for (auto flag: vector<string>{"-h", "--help"}) {
        cmdl({"banded_alignment", flag});
        ASSERT_EQ(cli(argc, argv), EXIT_SUCCESS);
        ASSERT_NE(stdout.str().find("banded_alignment"), string::npos);
    }
    return;
}
...
TEST_F(CliTest, version)
{
    ...
    for (auto flag: vector<string>{"-v", "--version"}) {
        cmdl({"banded_alignment", flag});
        ASSERT_EQ(cli(argc, argv), EXIT_SUCCESS);        
        ASSERT_NE(stdout.str().find("v0.1.0.0"), string::npos);
    }
    return;
}
...
TEST_F(CliTest, warn)
{
    for (auto flag: vector<string>{"-w", "--warn"}) {
        cmdl({"banded_alignment", flag, "info", "cmd1"});
        ASSERT_EQ(cli(argc, argv), EXIT_SUCCESS);
...
TEST_F(CliTest, subcommand)
{
    for (auto subcmd: vector<string>{"cmd1", "cmd2"}) {
        cmdl({"banded_alignment", "--warn=debug", subcmd});
        ASSERT_EQ(cli(argc, argv), EXIT_SUCCESS);        
